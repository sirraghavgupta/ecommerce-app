...
public class CategoryController {
    ...
    public ResponseEntity<VO> addMetaDataField(@RequestParam String fieldName) {
        return fieldService.addNewMetadataField(fieldName);
    }

    @GetMapping("/metadata-fields")
    public ResponseEntity<VO> getAllMetadataFields(@RequestParam(defaultValue = "0") String offset,
    ...
    public ResponseEntity<VO> addCategory(@RequestParam String categoryName,
    ...
    public ResponseEntity<VO> getCategoryDetails(@PathVariable(name = "id") Long categoryId) {
        return categoryService.getCategoryAllDetails(categoryId);
    }

    @GetMapping("/categories")
    public ResponseEntity<VO> getAllCategories(@RequestParam(defaultValue = "0") String offset,
                                               @RequestParam(defaultValue = "10") String size,
                                               @RequestParam(defaultValue = "id") String sortByField,
                                               @RequestParam(defaultValue = "ascending") String order) {
    ...
    public ResponseEntity<VO> deleteCategory(@PathVariable Long id){
        return categoryService.deleteCategoryById(id);
    }

    @PutMapping("/category/{id}")
    public ResponseEntity<VO> updateCategory(@PathVariable Long id, @RequestParam String name){
        return categoryService.updateCategory(id, name);
    }

    @PostMapping("/metadata-field-values")
    public ResponseEntity<VO> addMetadataFieldValues(@RequestBody MetadataFieldValueInsertDto fieldValueDtos){
        return categoryService.addMetadataFieldValuePair(fieldValueDtos);
    }

    @PutMapping("/metadata-field-values")
    public ResponseEntity<VO> updateMetadataFieldValues(@RequestBody MetadataFieldValueInsertDto fieldValueDtos){
        return categoryService.updateMetadataFieldValuePair(fieldValueDtos);
    }

    @GetMapping("/categories/seller")
    public ResponseEntity<VO> getAllCategories(){
        return categoryService.getAllCategoriesForSeller();
    }

    @GetMapping("/categories/customer")
    public ResponseEntity<VO> getAllCategories(@RequestParam(required = false) Long id){
